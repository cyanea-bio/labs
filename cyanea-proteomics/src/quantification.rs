...
pub fn spectral_counting(groups: &[ProteinGroup], psms: &[Psm]) -> Vec<ProteinQuant> {
    groups
        .iter()
        .map(|g| {
            let all_peptides: Vec<&str> = g
                .unique_peptides
                .iter()
...
pub fn intensity_quantification(
    ...
    let spec_map: std::collections::HashMap<&str, &MassSpectrum> =
        spectra.iter().map(|s| (s.id.as_str(), s)).collect();
...
impl TmtPlex {
    ...
    pub fn reporter_mzs(&self) -> Vec<f64> {
        match self {
            ...
            TmtPlex::Tmt10 => vec![
                126.1277, 127.1248, 127.1311, 128.1281, 128.1344, 129.1315, 129.1378, 130.1348,
                130.1411, 131.1382,
            ],
            TmtPlex::Tmt11 => {
            ...
            TmtPlex::Tmt16 => vec![
                126.1277, 127.1248, 127.1311, 128.1281, 128.1344, 129.1315, 129.1378, 130.1348,
                130.1411, 131.1382, 131.1445, 132.1416, 132.1479, 133.1449, 133.1512, 134.1483,
            ],
        }
    }
...
pub fn quantify_tmt(spectra: &[MassSpectrum], plex: TmtPlex, tolerance: f64) -> Vec<TmtQuant> {
    ...
    spectra
        ...
        .map(|spec| {
            ...
            for &reporter_mz in &reporters {
                let intensity = spec
                    .find_peak(reporter_mz, tolerance)
                    .map(|p| p.intensity)
                    .unwrap_or(0.0);
            ...
            let reporter_region_signal: f64 = spec
                .peaks
                .iter()
...
mod tests {
    ...
    fn test_quantify_tmt() {
        let peaks: Vec<Peak> = TmtPlex::Tmt6
            .reporter_mzs()
            .iter()
            .enumerate()
            .map(|(i, &mz)| Peak {
                mz,
                intensity: (i + 1) as f64 * 100.0,
            })
            .collect();
    ...
    fn test_log2_transform() {
        let mut quants = vec![ProteinQuant {
            accessions: vec!["P1".into()],
            raw_value: 8.0,
